...
CameraStream::CameraStream(QObject *parent) : QThread(parent)
{
	...
	connect(this, SIGNAL(startCamera(bool)), SLOT(onStartCamera(bool)), Qt::QueuedConnection);
}
...
void CameraStream::startPlay()
{
	m_isPlay = true;
	emit startCamera(true);
}

void CameraStream::stopPlay()
{
	m_isPlay = false;
	emit startCamera(false);
}

void CameraStream::setInitAV(bool val)
{
	m_InitAV = val;
}
...
void CameraStream::onStartCamera(bool val)
{
	if(!m_camera)
		return;
	if(val)
